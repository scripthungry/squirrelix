defmodule SquirrelixirIntegrationTest do
  ...
  These tests exercise the full Elixir path: temp Mix project → `Squirrelixir.generate/3`
  ...
  test "generate → compile → invoke with static metadata" do
    root = Squirrelixir.TestSupport.tmp_mix_project(:acorn_counter)
    ...
    assert Squirrelixir.generate(root, metadata,
    ...
    assert Squirrelixir.check(root, metadata,
             version: "v-integration",
             postgrex: IntegrationPostgrexMock
           ) == %CodegenCheckSummary{
             checked_count: 1,
  ...
  test "generate → compile → invoke with a query inferrer" do
    ...
    assert Squirrelixir.generate(root, inferrer,
             version: "v-integration",
             postgrex: IntegrationPostgrexMock
           ) == %CodegenSummary{
             generated_count: 1,
  ...
  test "generate with Postgres.inferrer → compile → invoke on live connection", %{conn: conn} do
    ...
    assert Squirrelixir.generate(root, Postgres.inferrer(conn), version: "v-integration") ==
