defmodule SquirrelixirGleamParityTest do
  @moduledoc """
  ...
  * `integration_test_` — Gleam compiles a throwaway Gleam project; Elixir uses
    `SquirrelixirIntegrationTest` with Mix projects and Postgrex mocks instead.
  """
  ...
  test "generate_module compiles when query uses a long postgres enum type name" do
    ...
    code =
      Codegen.generate_module(Squirrelixir.GleamParityLongEnumTest.SQL, [query],
        version: "v-test"
      )

    assert code =~ "required(:#{long_enum}) => String.t()"
    refute code =~ "# --- Enums ---"
    assert [{Squirrelixir.GleamParityLongEnumTest.SQL, _bytecode}] = Code.compile_string(code)
  end
